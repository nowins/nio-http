...
public class MonitoringIntegrationTest {
    ...
    void testMetricsEndpoint() throws Exception {
        ...
        assertTrue(response.contains("nio_http_requests_total"), "Metrics should contain request count");
    }
    ...
    void testMetricsTrackRequests() throws Exception {
        ...
        String response = sendRequest("GET /metrics HTTP/1.1\r\nHost: localhost\r\n\r\n");
        // Prometheus format: nio_http_requests_total{status="all"} N
        assertTrue(response.contains("nio_http_requests_total{status=\"all\"}"),
                "Metrics should contain total requests counter: " + response);
    }
